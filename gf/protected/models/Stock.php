...
//            return $prod.' --->'.$tot;
	...
	public function rules()
	{
		...
		return array(
			...
			array('nombre,buscarFactura,unidadMedida,codigoProducto', 'length', 'max'=>50),
			...
			array('idStock,limite, t.nombre,tipoProducto, detalle,buscarFactura, estado, codigoBarra, porcentajeIva, min, max, tipoProducto, idTipoProducto, idCuenta, idStockMarca', 'safe', 'on'=>'search'),
		);
	}
...
'params'=>array('')));
	...
	public function getPrecios($idStock)
	{
		$criteria=new CDbCriteria;
		$criteria->condition='t.idStock='.$idStock;
...
."  stock_tipoProducto.nombre as nombreTipoProducto, stock_marcas.nombreMarca as nombreMarca "
	...
	public function stockReal($todos=true,$codigoBarra='')
	{
		...
		{
			$criteria->compare('codigoBarra',$_GET['buscador'],true,'OR');
			$criteria->compare('t.nombre',$_GET['buscador'],true,'OR');
			$criteria->compare('stock_tipoProducto.nombre',$_GET['buscador'],true,'OR');
			$criteria->compare('stock_marcas.nombreMarca',$_GET['buscador'],true,'OR');
...
."  stock_tipoProducto.nombre as nombreTipoProducto, stock_marcas.nombreMarca as nombreMarca "
	...
	public function cambiarPrecios()
	{
		...
		$busca=isset($_GET['buscador'])?$_GET['buscador']:'';
		$buscaFactura=isset($_GET['buscarFactura'])?$_GET['buscarFactura']:'';
		$cantidadDigitos=Settings::model()->getValorSistema('DECIMALES_FACTURAS');
		if($buscaFactura!=''){
			return ComprasItems::model()->getStockFactura($buscaFactura);
		}else{
			$criteria->select = "t.*, SUBSTRING_INDEX( GROUP_CONCAT(CAST(stock_disponible.cantidadDisponible AS CHAR) ORDER BY stock_disponible.idStockDisponible desc), ',', 1 ) as cantidadDisponible,
			(select importeCompra from compras_items where idStock=t.idStock order by idCompraItem desc limit 1) as importeCompra ";
				$criteria->join = " LEFT JOIN stock_tipoProducto on stock_tipoProducto.idStockTipo = t.idTipoProducto 
				LEFT JOIN stock_marcas on stock_marcas.idStockMarcas = t.idStockMarca 
				LEFT JOIN stock_disponible on stock_disponible.idStock = t.idStock ";
			$criteria->compare('codigoBarra',$busca,true,'OR');
			$criteria->compare('t.nombre',$busca,true,'OR');
			$criteria->compare('stock_tipoProducto.nombre',$busca,true,'OR');
			$criteria->compare('stock_marcas.nombreMarca',$busca,true,'OR');
			$criteria->group='t.idStock';
        ...
        public function getPrecioVentaMin($id)
        {
            $sql="(SELECT  ROUND(importePesosStockMin*((stock.porcentajeIva/100)+1),2) as importe FROM stock_precios_items INNER JOIN stock on stock.idStock = stock_precios_items.idStock WHERE stock_precios_items.idStock = $id ORDER BY stock_precios_items.idStockPrecioStock desc LIMIT 1) ";
