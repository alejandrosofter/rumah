...
class TalonarioController extends RController
{
	...
	public function actionActualizarHojasIva($idTalonario)
	{
		$cantidadRegistrosHoja=60;
		$ano=Date('Y');
		$mes=Date('m');
		$mes--;
		$fechaInicio= $ano.'-'.$mes.'-01';
		$fechaFin=$ano.'-'.$mes.'-31';
		$facturas=Facturas::model()->consultarLibro($idTalonario,$fechaInicio,$fechaFin);
		$tal=Talonario::model()->findByPk($idTalonario);
		$tal->ultimoNroPagIva=$tal->ultimoNroPagIva+round(count($facturas)/$cantidadRegistrosHoja)+1;
		$tal->save();
		$this->redirect(array('/talonario/index','idPuntoVenta'=>1));
	}
