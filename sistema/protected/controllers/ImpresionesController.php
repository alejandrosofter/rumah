...
private function imprimirLibroIvaventas($fechaInicio,$fechaFin)
	{
		...
		$datosB=$model->consultarLibro('B',$fechaInicio,$fechaFin);
                $datosAnuladas=$model->consultarLibro('ANULADA',$fechaInicio,$fechaFin);
		
		Yii::import('application.vendors.PHPExcel',true);
        $objPHPExcel = new PHPExcel();
             
		$objPHPExcel=$this->setHojaLibroventas($objPHPExcel,$datosA,'FACTURAS A',0);
		$objPHPExcel->createSheet();
		$objPHPExcel=$this->setHojaLibroventas($objPHPExcel,$datosB,'FACTURAS B',1);
                $objPHPExcel->createSheet();
		$objPHPExcel=$this->setHojaLibroventas($objPHPExcel,$datosAnuladas,'ANULADAS',2);
...
fclose($fp);
	...
	private function setHojaLibroventas($objPHPExcel,$datos,$nombreHoja,$nroHoja)
	{
            
		$objPHPExcel->getDefaultStyle()->getFont()->setSize(10);
		$objPHPExcel->setActiveSheetIndex($nroHoja)
			->setCellValue('D2', 'LIBRO DE IVA VENTA')
...
$objPHPExcel->getActiveSheet()->getStyle('D2')->getFont()->setBold(true);
    ...
    for($i=0;$i<count($datos);$i++){
    	$fila=$i+6;
//    	$neto105=$datos[$i]['iva105']-($datos[$i]['iva105']*0.105);
//    	$neto21=$datos[$i]['iva21']-($datos[$i]['iva21']*0.21);

//   FORMAT_CURRENCY_USD_SIMPLE 	
    	$iva105=$datos[$i]['iva105']==0?0:-($datos[$i]['iva105']-($datos[$i]['importeFactura']-$datos[$i]['iva21']));
        $iva21=$datos[$i]['iva21']==0?0:-($datos[$i]['iva21']-($datos[$i]['importeFactura']-$datos[$i]['iva105']));
    	$objPHPExcel->setActiveSheetIndex($nroHoja)
            ->setCellValue('B'.$fila, $datos[$i]['numeroFactura'])
            ->setCellValue('C'.$fila, Yii::app()->dateFormatter->format("dd-M-y",$datos[$i]['fecha']))
            ->setCellValue('D'.$fila, $datos[$i]['cliente'])
            ->setCellValue('E'.$fila, $datos[$i]['cuit'])
            ->setCellValue('F'.$fila, $datos[$i]['iva105'])
            ->setCellValue('G'.$fila, $datos[$i]['iva21'])
            ->setCellValue('H'.$fila, $datos[$i]['iva105']*0.105)
            ->setCellValue('I'.$fila, $datos[$i]['iva21']*0.21)
            ->setCellValue('J'.$fila, ($datos[$i]['importeFactura']) )
           ;
...
$objPHPExcel->getActiveSheet()->getStyle('F6:J'.$filamasuno)->getNumberFormat()->setFormatCode(PHPExcel_Style_NumberFormat::FORMAT_CURRENCY_USD_SIMPLE);
...
$objPHPExcel->getActiveSheet()->getStyle('B6:J'.$filamasuno)->applyFromArray($styleArray2);
...
private function imprimirIvaCompras($fechaInicio,$fechaFin)
	{
		...
		$objPHPExcel=$this->setHojaLibroCompras($objPHPExcel,$datosA,'FACTURAS A',0);
		$objPHPExcel->createSheet();
		$objPHPExcel=$this->setHojaLibroCompras($objPHPExcel,$datosB,'FACTURAS B',1);
	...
	private function setHojaLibroCompras($objPHPExcel,$datos,$nombreHoja,$nroHoja)
	{
		$objPHPExcel->setActiveSheetIndex($nroHoja);
		...
		$objPHPExcel->getDefaultStyle()->getFont()->setSize(10);
		$objPHPExcel->setActiveSheetIndex($nroHoja)
		 ->setCellValue('D2', 'LIBRO IVA COMPRAS')
            ->setCellValue('B5', 'Nro Fact.')
            ->setCellValue('C5', 'Fecha')
            ->setCellValue('D5', 'Razón Social')
            ->setCellValue('E5', 'Cuit')
...
$fila=0;
    ...
    for($i=0;$i<count($datos);$i++){
    	...
    	$objPHPExcel->setActiveSheetIndex($nroHoja)
            ->setCellValue('B'.$fila, $datos[$i]['numeroFactura'])
            ->setCellValue('C'.$fila, $datos[$i]['fecha'])
            ->setCellValue('D'.$fila, $datos[$i]['nombreProveedor'])
            ->setCellValue('E'.$fila, $datos[$i]['cuitProveedor'])
            ->setCellValue('F'.$fila, ($datos[$i]['importe105']))
            ->setCellValue('G'.$fila, ($datos[$i]['importe21']))
            ->setCellValue('H'.$fila, ($datos[$i]['importe105'])*0.105)
            ->setCellValue('I'.$fila, ($datos[$i]['importe21'])*0.21)
            ->setCellValue('J'.$fila, $datos[$i]['importe'])
            ;
    }
if($fila!=0){
	$objPHPExcel->getActiveSheet()->getColumnDimension('A')->setWidth(3);
...
//					"	<td>".$items['cuit']."</td>".
	...
	private function getDetalleVentas($items)
	{
		...
		foreach($items as $item){
                    $saldo+=$item['importeTotal'];
			$salida.="<tr>" .
					"	<td>".$item['cantidad']."</td>".
					"	<td>".Yii::app()->numberFormatter->formatCurrency($item['importe'],'$')."</td>".
                                        "	<td>".Yii::app()->numberFormatter->formatCurrency($item['importeTotal'],'$')."</td>".
	...
	private function getInformeVentas($items)
	{
		...
		$saldo=0;
		foreach($items as $item){
//			$cuenta=($item['cuenta']==NULL)?'SERVICIOS':$item['cuenta'];
                    $saldo+=$item['importe'];
			$salida.="<tr>" .
