...
$auxIni = explode("-",$fechaInicio);
		...
		$connection=Yii::app()->getDb();
        $command=$connection->createCommand("
SELECT SUM(itemsFacturaSaliente.cantidad) as cantidad, AVG( itemsFacturaSaliente.importeItem*((stock.porcentajeIva/100)+1) ) as importe ,SUM( (itemsFacturaSaliente.importeItem*((stock.porcentajeIva/100)+1)) *itemsFacturaSaliente.cantidad ) as importeTotal , itemsFacturaSaliente.nombreItem
...
WHERE t.fecha
BETWEEN  '$auxIni'
AND  '$auxFin' AND t.estado<>'ANULADA'
GROUP BY itemsFacturaSaliente.idElemento
...
$auxIni = explode("-",$fechaInicio);
		...
		$connection=Yii::app()->getDb();
        $command=$connection->createCommand("
SELECT SUM( (itemsFacturaSaliente.importeItem*((stock.porcentajeIva/100)+1))*itemsFacturaSaliente.cantidad ) AS importe,t.tipoFactura
FROM  `facturasSalientes` t
LEFT JOIN itemsFacturaSaliente ON itemsFacturaSaliente.idFacturaSaliente = t.idFacturaSaliente
LEFT JOIN stock ON stock.idStock = itemsFacturaSaliente.idElemento
WHERE t.fecha BETWEEN CAST('$auxIni' AS DATETIME)
AND CAST('$auxFin' AS DATETIME) 
                AND t.estado<>'ANULADA' GROUP BY t.tipoFactura");
...
$auxIni = explode("-",$fechaInicio);
		...
		$connection=Yii::app()->getDb();
        $command=$connection->createCommand("
SELECT ROUND(SUM( ordenesCobroFacturas.importeCobroFactura),2) AS importe,formasDePago.nombreForma
FROM  `ordenesCobro` t
...
WHERE t.fechaOrdenCobro BETWEEN '$auxIni' 
AND '$auxFin' group by t.idFormaPago");
