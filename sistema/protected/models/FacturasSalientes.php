...
bsaaaaaaaceeeeiiiidnoooooouuuyybyRr';
    ...
    return utf8_encode($cadena);
        ...
        private function getItemFiscal($tipo,$item,$importe,$cantidad,$iva)
        {
            ...
            $sal="";
            if($importe<0)
                 $sal= "@FACTPAGO|DESCUENTO |".(-$importe)."|D,";
            else{
            if($tipo=="CREDITOB"){
                $iva=$iva/100;
...
//            echo $sql;
	...
	{
		$sql="SELECT t.*,clientes.cuit FROM facturasSalientes_view t LEFT JOIN clientes on clientes.idCliente = t.idCliente WHERE t.fecha BETWEEN '$fechaInicio 00:00:00' AND '$fechaFin 00:00:00' AND t.estado<>'ANULADA'";
                $connection=Yii::app()->getDb();
                $command=$connection->createCommand($sql);
                ...
                foreach ($res as $value){
                    $i++;
                    $texto.=$this->getLineaCitiVentas($value,($i==count($res)));
        ...
        private function getLineaCitiVentas($datos,$ultimo)
        {
           ...
           $nroDeComprobante=321;
            ...
            $linea="";
            $linea.=11; //codigo del comprobante
            $linea.=str_replace('-', '', $fecha[0]); // fehca
            $linea.=str_pad('',16,0, STR_PAD_LEFT); // tipo de comprobante
            ...
            $linea.=$codigoDocumento.''; // nro de identificacion del comprador 2
        ...
        private function getLineaCitiVentas2($datos,$ultimo)
        {
            ...
            $fecha=explode(' ', $datos['fecha']);
            $n=explode('-', $datos['numeroFactura']);
            $num=str_replace(' ','',$n[1]);
            $nroFactura=str_pad($num,20,'0', STR_PAD_LEFT);
            $esConsumidorFinal= $this->esConsumidorFinal($datos['idCliente']);
            $cuit=str_replace(' ','',str_replace('-', '', $datos['cuit']));
            $codigoDocumento=$this->getTipoDocCliente($datos['tipoFactura'],$datos['idCliente']); // no se que numero poner cuando no es factura A ni B
           $importeNeto=number_format($datos['iva105']+$datos['iva21'],2,'','')+'';
           $importeTotal=number_format($datos['importeFactura'],2,'','')+'';
           
            $linea="";
            $linea.=1; //tipo de registro - 1
            $linea.=str_replace('-', '', $fecha[0]); // fecha 9
            $linea.=$this->getTipoComprobante($datos['talonarioId'],$esConsumidorFinal); // tipo de comprobante
...
SELECT COUNT(t.idFacturaVencimiento) as cantidad, SUM(t.importeFacturaSaliente) as importe FROM facturasSalientesVencimiento t WHERE t.estado='PEND' AND (t.fechaVencimiento $signo CURDATE() AND t.fechaVencimiento<DATE_ADD(CURDATE(),INTERVAL $dias DAY))";
	...
	public function search($esFacturaCredito=0,$estado='')
	{
		...
		$criteria->compare('fechaEstimadaCobro',$this->fechaEstimadaCobro,true);
		$criteria->select = "t.*,talonario.esElectronica,(SELECT e.estado FROM facturasSalientes_respuestaElectronica e WHERE e.idFacturaSaliente =t.idFacturaSaliente ORDER BY e.idFacturaRespuesta desc LIMIT 1) as estadoElectronica,SUM(IF(itemsFacturaSaliente.porcentajeIva='21',
							itemsFacturaSaliente.importeItem*itemsFacturaSaliente.cantidad,0)) as iva21,
...
8: Nota de Crédito B
        ...
        public function getTipoComprobante($idTalonario,$consumidorFinal=false,$salidaTexto=true)
        {
            ...
            $tal=Talonario::model()->findByPk($idTalonario);
            if($tal==null)return '83';
            if($tal->tipoTalonario=='Factura'&&$tal->letraTalonario=='B')if($consumidorFinal)$sal= '82';else $sal= '06';
            if($tal->tipoTalonario=='Factura'&&$tal->letraTalonario=='A')if($consumidorFinal)$sal= '81';else $sal= '01';
            if($tal->tipoTalonario=='Factura'&&$tal->letraTalonario=='C')if($consumidorFinal)$sal= '83';else $sal= '83';
            if($tal->tipoTalonario=='Nota Credito'&&$tal->letraTalonario=='B')$sal= '08';
            if($tal->tipoTalonario=='Nota Credito'&&$tal->letraTalonario=='A')$sal= '03';
        ...
        public function getPuntoVenta($idTalo)
        {
            ...
            if($tal==null) return 0;
            return $tal->codigoPuntoVenta;
        }
