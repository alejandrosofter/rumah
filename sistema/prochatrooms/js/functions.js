...
function initAll()
{
	...
	{
		createMessageDiv('0',uID,displayMDiv,2,entryNotice,'beep_high.mp3','','');
		roomlogout();
		roomlogin();
	}
...
{
	document.getElementById(ndiv).innerHTML  = '<span alt="'+lang52+'" title="'+lang52+'" id="smilies" class="iconSmilies" onclick="doSmilies(\''+nBar+'\',\''+nContainer+'\',\''+nWin+'\');"></span>';
	document.getElementById(ndiv).innerHTML += '<span alt="'+lang53+'" title="'+lang53+'" id="ringbell" class="iconRingbell" onclick="ringBell(\''+nBar+'\',\''+nContainer+'\')"></span>';
	document.getElementById(ndiv).innerHTML += '<span alt="'+lang54+'" title="'+lang54+'" id="style" class="iconStyle" onclick="doStyles(\''+nBar+'\',\''+nContainer+'\',\''+nWin+'\');"></span>';
	document.getElementById(ndiv).innerHTML += '<span alt="'+lang55+'" title="'+lang55+'" id="avatar" class="iconAvatar" onclick="doAvatars(\''+nBar+'\',\''+nContainer+'\',\''+nWin+'\')"></span>';

	if(mySFX[0])
	{
		document.getElementById(ndiv).innerHTML += '<span alt="'+lang56+'" title="'+lang56+'" id="sounds" class="iconSounds" onclick="doSFX(\''+nBar+'\',\''+nContainer+'\',\''+nWin+'\')"></span>';
	}

	if(ndiv.search("pmenuBar_"))
	{
		document.getElementById(ndiv).innerHTML += '<span alt="'+lang57+'" title="'+lang57+'" id="rubber" class="iconRubber" onclick=\'clrScreen();\'></span>';
		document.getElementById(ndiv).innerHTML += '<span alt="'+lang58+'" title="'+lang58+'" id="edit" class="iconEdit" onclick=\'editSettings();\'></span>';

	}

	document.getElementById(ndiv).innerHTML += '<span alt="'+lang59+'" title="'+lang59+'" id="transcripts" class="iconTranscripts" onclick=\'showInfoBox("viewTranscripts","400","600","200","index.php?transcripts=1&roomID="+roomID,"");\'></span>';
	document.getElementById(ndiv).innerHTML += '<span alt="'+lang60+'" title="'+lang60+'" id="help" class="iconHelp" onclick=\'newWin("help/index.php")\'></span>';

	/* do not edit */
	if(showCopyright)
	{
		document.getElementById(ndiv).innerHTML += '<span alt="'+lang63+'" title="'+lang63+'" id="copyright" class="iconCopyright" onclick=\'showInfoBox("copyRight","220","300","200","",copyRight());\'></span>';
	}
}
...
function logout(id)
{
	window.location.replace("index.php?logout="+id);
}
...
function showInfoBox(info,height,width,top,url,txt)
{
	...
	if(txt)
	{
		...
		newdiv.innerHTML  = "<div class=\"userInfoTitle\" style=\"padding-top:3px;cursor:move;\" onmousedown='dragStart(event,\""+info+"\");'><b>"+lang4+"</b></div>";
...
{
	var html = '';
		html += '<div style="text-align:left;padding-left:15px;padding-top:10px;">';
		html += 'Software: Pro Chat Rooms<br>';
		html += 'Version: '+version+'<br>';
		html += 'Developers: Pro Chat Rooms<br><br>';
		html += 'Url: <a href="http://prochatrooms.com" target="_blank">http://prochatrooms.com</a><br>';
		html += 'Support: <a href="http://support.prochatrooms.com" target="_blank">http://support.prochatrooms.com</a><br><br>';
		html += '&copy;'+new Date().getFullYear()+' - All Rights Reserved.';
		html += '</div>';
